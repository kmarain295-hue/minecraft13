...
export function createBulletSystem(
  ...
  function update(dt: number, camera: THREE.Camera) {
    ...
    for (const r of rockets) {
      ...
      // detonate on terrain contact OR on any solid cave piece (walls,
      // columns, rubble) — no more flying through stone ruins
      if (
        r.life <= 0 ||
        rp.y <= rGround + 2.5 ||
  ...
  return { fireFrom, fireRocket, update, setSlowmo, detonate, stats, dispose };
}
